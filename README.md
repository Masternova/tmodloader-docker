...
As a disclaimer, I am new to github and didn't know how to correctly fork from rfvgyhn/tmodloader. All credit to him for all of this work, I just updated it to the newest version of tModloader
...
## Quick Start
    ...
    docker run -d --name tmod -p 7777:7777 -v /etc/localtime:/etc/localtime:ro codyruggiero/tmodloader-docker

# Adding worlds

    docker run -d --name tmod -p 7777:7777 -v /etc/localtime:/etc/localtime:ro -v $(pwd)/data:/terraria codyruggiero/tmodloader-docker
...
You can mount a config file. This allows you to specify server and world settings. If you don't specify one, a [default] will be used. See [wiki] for file format details.
    ...
    docker run -d --name tmod -p 7777:7777 -v /etc/localtime:/etc/localtime:ro -v $(pwd)/data:/terraria -v $(pwd)/config.txt:/terraria-server/config.txt codyruggiero/tmodloader-docker
...
If you want to use the server's mod browser to install and enable mods, run an interactive container with the `setup` parameter appended to the end.
    ...
    docker run -it --rm -v $(pwd)/data:/terraria codyruggiero/tmodloader-docker setup
...
TMOD_IDLE_CHECK_OFFSET   | 0              | This allows for sub-minute resolution if the idle check interval is specified
    ...
    docker run -d --name tmod \
               ...
               codyruggiero/tmodloader-docker
...
# Sample Docker Compose
    ...
    services:
        tmod:
            image: 'codyruggiero/tmodloader-docker:latest'
            container_name: 'tmod'
            ports:
                - '7777:7777'
            volumes:
...
[default]: https://github.com/codyruggiero/tmodloader-docker-docker/blob/master/config.txt
...
[0]: https://hub.docker.com/r/codyruggiero/tmodloader-docker
